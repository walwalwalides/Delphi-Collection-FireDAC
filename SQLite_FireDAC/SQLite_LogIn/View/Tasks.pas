...
uses
  ...
  Utils, Declare, UOptions, Language, Language_Lib, StringUtils, DMTasks;
...
begin
  ...
  Width := nFRMWIDTH;
  Caption := Lang.sTasksTitle;
  LabelTasksList.Caption := Lang.sTasksList;
  ButtonOk.Caption := LangLib.sBut_Close;
  SetTasksList;
...
var
  i: integer;
begin
  DataModuleTasks.GetAllTasks(AppliData.nUserID);
  EditTask.Clear;
  ListBoxTasks.Clear;
...
begin
  ...
  begin
    MessageDlg(Lang.sMsgTaskMax, mtWarning, [mbOk], 0);
...
begin
  ...
  begin
    ...
    if not bFound then
      if DataModuleTasks.AddTask(AppliData.nUserID, EditTask.Text) = TRUE then
        SetTasksList;
  end;
end;
...
begin
  ...
  begin
    ...
    if DataModuleTasks.UpdTask(nID, EditTask.Text) = TRUE then
      SetTasksList;
  end;
end;
...
begin
  ...
  begin
    if HrsUtils.TaskUsed(AppliData.anTasksID[ListBoxTasks.ItemIndex]) then
      MessageDlg(Lang.sMsgTaskUsed, mtWarning, [mbOk], 0)
    else
    begin
      nID := AppliData.anTasksID[ListBoxTasks.ItemIndex];
      if DataModuleTasks.DelTask(nID) = TRUE then
      begin
        SetTasksList;
